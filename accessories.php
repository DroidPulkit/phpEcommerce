...
$sql = "SELECT COUNT(id) AS 'COUNT' FROM `product` WHERE `category_id` = 3";
...
$sqlForProducts = "SELECT * FROM `product` WHERE `category_id` = 3";
                            ...
                            <?php 
                                ...
                                if (mysqli_num_rows($resultForProducts) > 0) {
                                    while ($row = mysqli_fetch_assoc($resultForProducts)) {
                                        $data1 = '<div class="col-12 col-sm-6 col-lg-4"><div class="single-product-wrapper"><!-- Product Image --><div class="product-img"><img src="img/products/accessories/';
                                        $pictures = $row['picture'];
                                        $picture = explode(", ", $pictures);
                                        $data2 = $picture[0];
                                        $product_id = $row["id"];
                                        $data3 = '.jpg" alt=""></div><!-- Product Description --><div class="product-description"><a href="view.php?category_id=3&product_id=' . $product_id . '"><h6>';
                                        $data4 = $row["name"];
